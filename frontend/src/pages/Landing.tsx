      ...
      <div className="mx-auto grid max-w-6xl items-center gap-12 px-6 pb-20 pt-16 lg:grid-cols-[1.1fr_1fr]">
        <div>
          ...
          <p className="lp-fade-3 mt-5 max-w-lg text-[17px] leading-relaxed" style={{ color: "var(--muted)" }}>
            Pulse watches your Square, Stripe, or CSV data, flags regulars who are quietly slipping away —
...
function RiskDemo() {
  ...
  const band =
    ratio >= 2.5
      ? { label: "Needs Attention", color: "#A23B1E", bg: "#F7E3DC", action: "Pulse drafts a win-back email — you tap approve." }
      : ratio >= 1.5
      ? { label: "Keep an Eye On", color: "#A9781F", bg: "#F4EAD1", action: "Pulse watches daily and flags them the moment risk rises." }
    ...
    <section id="demo" className="mx-auto max-w-6xl scroll-mt-20 px-6 py-20">
      <div className="lp-reveal">
        <SectionHead
          ...
          sub="Drag the slider — Pulse scores churn risk from each customer's own rhythm, and explains it in plain English."
        />
      </div>
...
function HowItWorks() {
  const steps = [
    { n: "01", title: "Connect in 2 minutes", body: "Link Square or Stripe, or just upload a customer CSV. No IT department required — if you can attach a file to an email, you can set up Pulse." },
    ...
    { n: "03", title: "Approve the win-back", body: "Pulse drafts a personalized email mentioning their favorite order. You tap approve, it sends, and recovered visits are tracked back to the message." },
  ];
  return (
    ...
    <footer className="border-t py-10" style={{ borderColor: "var(--border)", background: "var(--surface)" }}>
      <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-4 px-6 sm:flex-row">
        <div className="flex items-center gap-2">
          <span className="font-logo inline-flex h-6 w-6 items-center justify-center rounded-full text-[13px]" style={{ background: "var(--ink-strong)", color: "var(--cream-text)" }}>P</span>
          <span className="font-display text-base font-bold" style={{ color: "var(--ink)" }}>Pulse</span>
